...
function createStore(dbPath, defaults) {
  ...
  const stmt = {
    ...
    updateCard: db.prepare('UPDATE cards SET title = ?, type = ?, focused = ?, subtasks = ?, priority = ? WHERE id = ? AND user_id = ?'),
    ...
    archiveCard: db.prepare('UPDATE cards SET archived = 1, focused = 0 WHERE id = ? AND user_id = ?'),
    restoreCard: db.prepare('UPDATE cards SET archived = 0, focused = 0, position = ? WHERE id = ? AND user_id = ?'),
  ...
  const rowToCard = (r) => ({ id: r.id, boardId: r.board_id, title: r.title, type: r.type, focused: !!r.focused, position: r.position, subtasks: safeJson(r.subtasks, []), priority: r.priority || 0, createdAt: r.created_at });
  ...
  return {
    ...
    deleteUser(id) { stmt.deleteUserCards.run(id); stmt.deleteUserBoards.run(id); stmt.deleteUserHistory.run(id); stmt.deleteUserRecovery.run(id); stmt.deleteUser.run(id); },
    ...
    updateCard(userId, id, fields) {
      ...
      stmt.updateCard.run(title, type, focused, subtasks, priority, id, userId);
      return rowToCard(stmt.cardById.get(id, userId));
    },
    ...
    archiveCard(userId, id) { const r = stmt.cardById.get(id, userId); if (!r) return null; stmt.archiveCard.run(id, userId); return rowToCard(stmt.cardById.get(id, userId)); },
